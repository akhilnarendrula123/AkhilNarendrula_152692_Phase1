			...
			case 1:
				...
				try {
					if (service.validateDetails(customerBean)) {
						...
						System.out.println("Account created successfully with contact number:" + contact);
					} else
			...
			case 4:
				...
				try {
					if (service.fundTransfer(senderMobNum, receiverMobNum, transferAmount)) {
						System.out.println( " "+ transferAmount + " Succesfully Transfered to " + receiverMobNum);
						Customer bean4 = service.showBalance(senderMobNum);
						System.out.println("Updated Balance in Sender's Account is " + bean4.getBalance());
						bean4 = service.showBalance(receiverMobNum);
						System.out.println("Updated Balance in Receiver's Account is  " + bean4.getBalance());
					}
				} catch (PaymentWalletException paymentWalletException) {
					System.out.println(paymentWalletException.getMessage());
				}
